...
`riseballs-live` never reads or writes the DB. It has no `DATABASE_URL` in its container, no JDBC driver, no knowledge of the `teams` or `games` tables. Its slug resolution is driven by two classpath resources baked into the JAR at build time: `espn_slug_overrides.json` (163 entries: a one-shot snapshot of the `ESPN_SLUG_OVERRIDES` hash that lived on the Ruby `EspnScoreboardService` before that service was deleted 2026-04-19, plus three reviewer-added entries — Florida Atlantic, Sam Houston, San Jose State accent) and `known_slugs.txt` (594 D1/D2 slugs exported once from `Team.pluck(:slug)`). The Ruby map no longer exists in the Rails repo; this JSON file is the single remaining home for those overrides. See [live/02-architecture.md](../live/02-architecture.md) "SlugResolver" for update workflow.
...
The first two consult the `team_aliases` table in Postgres. `riseballs-live` has no DB access — instead, it loads classpath resources at startup: `espn_slug_overrides.json` (163 entries) and `known_slugs.txt` (594 D1/D2 slugs). The JSON is a one-time snapshot of the old Ruby `ESPN_SLUG_OVERRIDES` hash; the Ruby service that defined that hash (`EspnScoreboardService`) was deleted 2026-04-19, so the JSON resource in the `riseballs-live` repo is now the single canonical home for ESPN-location-to-slug overrides. Updating the slug list now requires an edit in `riseballs-live/src/main/resources/espn_slug_overrides.json` + rebuild + redeploy. If a team's canonical slug changes in Rails, `known_slugs.txt` has to be re-exported and committed.
