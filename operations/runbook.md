...
| Live overlay not updating on Scoreboard | [#live-overlay-not-updating](#live-overlay-not-updating) |
...
## Live overlay not updating

**Symptom:** Scoreboard shows Rails-sourced upcoming times for games that are actually live, or stale innings for games in progress. The live score overlay is silent.

**Context:** Since 2026-04-19, live scores on `/scoreboard` come from the `riseballs-live` service (not Rails, not StatBroadcast — those are all gone). The frontend fetches Rails and `live.riseballs.com/scoreboard` in parallel; if `riseballs-live` is slow, erroring, or returning no events, Rails data is all the page has.

**Diagnose:**

```sh
# 1. Is the service up?
curl -sI https://live.riseballs.com/health

# 2. What is it returning for today?
curl -s "https://live.riseballs.com/scoreboard?date=$(date +%Y-%m-%d)" | jq '.source, (.events | length), .fetchedAt'
# source: "fresh" | "cache" | "stale"
# events: integer

# 3. Structured access log tail (per request: date, source, events, upstream_ms_ncaa, upstream_ms_espn, total_ms)
ssh dokku@ssh.mondokhealth.com logs riseballs-live -t
```

Common causes:

1. **NCAA + ESPN both timing out.** `total_ms >= 12000` with `source: "stale"` or a 504 means the 12s joint timeout fired. Usually resolves itself when the upstreams recover; no action needed unless it's persistent.
2. **CORS misconfigured.** Browser console shows CORS errors. Verify the allowlist in `WebConfig.java` still matches the site's origin. Only `https://riseballs.com`, `https://www.riseballs.com`, `http://localhost:3000`, `http://localhost:3100` are permitted. Any new preview domain needs a code change + redeploy.
3. **Rate-limited IP.** Browser's IP hit 60 requests/min. Expect 429s with `Retry-After: 1`. Real users shouldn't trip this; if they are, the frontend polling cadence (30s) is probably misconfigured.
4. **Slug resolver miss.** The event is there but `ncaaContestId`/`homeSlug`/`awaySlug` don't match any Rails game. Check `espn_slug_overrides.json` or `known_slugs.txt` in the `riseballs-live` repo — may need a new mapping. Both files are baked into the JAR; fixing requires a rebuild + deploy.
...
# If it's a code issue (CORS, slug override), edit in the riseballs-live repo and redeploy:
cd /path/to/riseballs-live
...
```sh
curl -s "https://live.riseballs.com/scoreboard?date=$(date +%Y-%m-%d)" | jq '.events[] | {ncaaContestId, homeSlug, awaySlug, state}' | head
```

The live overlay is transient — there's no DB fallback. If it's broken, Rails data is the only thing the user sees, and finals will display correctly either way. The overlay only affects in-progress games.

**See:** [live/00-overview.md](../live/00-overview.md), [live/02-architecture.md](../live/02-architecture.md).
...
# riseballs-live logs (structured JSON per request)
ssh dokku@ssh.mondokhealth.com logs riseballs-live -t
